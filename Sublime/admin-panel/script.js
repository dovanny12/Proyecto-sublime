...
function setupInventoryListeners() {
    ...
    // Consumir modal
    const consumirModal = document.getElementById('consumirMateriaModal');
    const cerrarConsumirModal = () => { if (consumirModal) consumirModal.classList.remove('active'); };
    ...
    const consumirCantidadEl = document.getElementById('consumirCantidad');
    if (consumirCantidadEl) {
        consumirCantidadEl.addEventListener('input', () => {
            if (!consumirMateriaCurrentId) return;
            const item = windowMateriaPrimaItems.find(m => m.id_materia === consumirMateriaCurrentId);
            if (!item) return;
            ...
            const cant = parseInt(consumirCantidadEl.value) || 0;
            const preview = document.getElementById('consumirPreview');
            if (cant > 0) {
                preview.style.display = 'block';
                document.getElementById('consumirPreviewStock').textContent = stockDisp;
                document.getElementById('consumirPreviewQty').textContent = `-${cant}`;
                const restante = stockDisp - cant;
                const remEl = document.getElementById('consumirPreviewRemaining');
                remEl.textContent = restante < 0 ? '⚠ Stock insuficiente' : restante;
                remEl.style.color = restante < 0 ? '#ff416c' : restante === 0 ? '#ffb700' : 'var(--primary)';
            } else {
    ...
    // Confirm consumir
    const confirmConsumirBtn = document.getElementById('confirmConsumirMateriaBtn');
    if (confirmConsumirBtn) {
        confirmConsumirBtn.addEventListener('click', async () => {
            if (!consumirMateriaCurrentId) return;
            ...
            try {
                const res = await apiRequest(`materia-prima/${consumirMateriaCurrentId}/consumir`, {
                    method: 'POST',
                    body: { cantidad: cant, motivo, notas }
                });
...
window.openConsumirMateria = function (id) {
    ...
    document.getElementById('consumirMateriaInfo').innerHTML = `
        ...
        <div class="consumir-materia-info-row">
            ...
            <span>Stock disponible: <strong style="color:var(--primary);">${stockDisp}</strong> de ${item.unidades} unidades</span>
        </div>
        <div class="consumir-materia-info-row">
            <i class="fa-solid fa-dollar-sign"></i>
            <span>Precio unitario: $${parseFloat(item.precio_unitario).toFixed(2)} &nbsp;|&nbsp; Total: $${parseFloat(item.subtotal).toFixed(2)}</span>
        </div>`;

    document.getElementById('consumirCantidad').value = '';
    document.getElementById('consumirNotas').value = '';
    document.getElementById('consumirMotivo').value = 'personalizacion';
    document.getElementById('consumirPreview').style.display = 'none';
    ...
    if (currentInventorySubTab === 'materia-prima') {
        ...
        if (filtered.length === 0) {
            materiaBody.innerHTML = `
                <tr>
                    <td colspan="13" style="text-align:center;color:var(--muted);padding:30px;">
        ...
        materiaBody.innerHTML = filtered.map(item => {
            ...
            return `
                <tr>
                    ...
                    <td>${ivaPct.toFixed(1)}%</td>
                    <td style="color:#ffb700;font-weight:600;">$${ivaUsd.toFixed(2)}</td>
                    <td style="color:#ffb700;font-weight:600;">Bs ${ivaBs.toFixed(2)}</td>
                    <td><strong style="color:var(--primary);">$${subUsd.toFixed(2)}</strong></td>
                    <td><strong>Bs ${totalBsRow.toFixed(2)}</strong></td>
...
function updateOrdersStats(orders) {
    ...
    if (pendingEl) pendingEl.textContent = counts.pendiente + counts.procesando;
}
...
function renderOrdersList() {
    ...
    if (ordersStatusFilter !== 'Todos') {
        filtered = filtered.filter(o => o.estado === ordersStatusFilter);
    }

    if (query) {
        filtered = filtered.filter(o =>
            (o.cliente || '').toLowerCase().includes(query) ||
            (o.correo || '').toLowerCase().includes(query) ||
    ...
    if (!filtered.length) {
        container.innerHTML = `
            <div class="orders-empty">
                ...
                <p>${allOrders.length === 0 ? 'No hay pedidos registrados.' : 'Sin resultados para tu búsqueda.'}</p>
            </div>`;
        return;
    }
    ...
    container.innerHTML = filtered.map(order => {
        ...
        if (order.estado === 'Pendiente de Verificación') {
            actionsHtml = `
                <button class="btn-verify-order" onclick="verifyOrder(${order.id})">
                    <i class="fa-solid fa-circle-check"></i> Verificar Pago
                </button>`;
        } else if (order.estado === 'Procesando') {
            actionsHtml = `
                <input type="text" id="tracking-${order.id}" placeholder="N° guía">
                <button class="btn-ship-order" onclick="shipOrder(${order.id})">
                    <i class="fa-solid fa-paper-plane"></i> Marcar Enviado
                </button>`;
        } else if (order.estado === 'Enviado') {
            actionsHtml = `
                <button class="btn-deliver-order" onclick="deliverOrder(${order.id})">
                    <i class="fa-solid fa-house-chimney-user"></i> Marcar Entregado
                </button>`;
        }

        /* ── Comprobante ── */
        const comprob = order.comprobante_pago
            ? `<div class="comprobante-preview">
                   <span class="comprobante-preview-label">Comprobante</span>
                   <a href="../static/images/payments/${order.comprobante_pago}" target="_blank">
                       <img src="../static/images/payments/${order.comprobante_pago}" class="comprobante-thumb" alt="Comprobante">
                   </a>
               </div>`
            : '';

        return `
        <div class="order-card-modern" style="border-left-color:${color};">

            <!-- Header -->
            <div class="order-card-header">
                <div class="order-header-left">
                    <input type="checkbox" class="order-select-checkbox" data-id="${order.id}">
                    <span class="order-id">#${order.id}</span>
                    <span style="font-size:0.78rem;color:var(--text-muted);font-weight:600;">
                        ${(order.items_count || 0)} producto(s)
                    </span>
                </div>
            ...
            <div class="order-card-body">
                ...
                <div class="order-body-col">
                    ...
                    <p><i class="fa-solid fa-wallet"></i> ${order.metodo_pago || 'N/A'}</p>
                    <p><i class="fa-solid fa-hashtag"></i> ${order.referencia_pago || 'N/A'}</p>
                    ${comprob}
                </div>

                <!-- Col 4: Productos comprados -->
                <div class="order-items-col">
                    <h4><i class="fa-solid fa-bag-shopping"></i> Productos del pedido</h4>
            ...
            <div class="order-card-footer">
                <div class="order-total-display">
                    ...
                    <strong>$${usd}</strong>
                    <span class="order-total-bs">Bs ${bs}</span>
                </div>
    ...
    container.querySelectorAll('.order-select-checkbox').forEach(cb => {
        cb.addEventListener('change', e => {
            ...
            if (e.target.checked) {
                if (!selectedOrderIds.includes(orderId)) selectedOrderIds.push(orderId);
            } else {
    ...
    /* Already-selected checkboxes stay checked after re-render */
    selectedOrderIds.forEach(id => {
        const cb = container.querySelector(`.order-select-checkbox[data-id="${id}"]`);
        if (cb) cb.checked = true;
    });

    /* Update dashboard recent orders widget */
    updateDashboardRecentOrders(allOrders);
}
...
document.addEventListener('click', e => {
    const pill = e.target.closest('.order-filter-pill');
    if (!pill) return;
    document.querySelectorAll('.order-filter-pill').forEach(p => p.classList.remove('active'));
    pill.classList.add('active');
    ordersStatusFilter = pill.dataset.status;
    renderOrdersList();
});

/* ── Search input listener ── */
document.addEventListener('input', e => {
    if (e.target.id === 'ordersSearchInput') renderOrdersList();
});
...
async function shipOrder(orderId) {
    ...
    try {
        await apiRequest('order/' + orderId + '/status', { method: 'PUT', body: data });
...
async function deliverOrder(orderId) {
    ...
    try {
        await apiRequest('order/' + orderId + '/status', { method: 'PUT', body: { estado: 'Entregado' } });
...
async function loadSalesData() {
    try {
        ...
        clientSelect.innerHTML = `
            ...
            ${response.clients.map(c => `<option value="${c.id_cliente}">${c.nombre}</option>`).join('')}
        `;

        // Store products globally and render
        allSalesProducts = response.products || [];
        renderSalesProducts();
        buildCategoryPills();

        // Client change handler
        clientSelect.addEventListener('change', () => loadClientCart());
        ...
        showToast('Error cargando datos de ventas: ' + error.message, 'error');
    }
}

function buildCategoryPills() {
    const container = document.getElementById('salesCatFilters');
    if (!container) return;

    const cats = ['Todos', ...new Set(allSalesProducts.map(p => p.categoria || 'General').filter(Boolean))];

    container.innerHTML = cats.map(cat => `
        <button class="sales-cat-pill ${cat === salesCatFilter ? 'active' : ''}"
                onclick="setSalesCat('${cat}')">
            ${cat}
        </button>
    `).join('');
}
...
function renderSalesProducts() {
    ...
    const query = (document.getElementById('salesProductSearch')?.value || '').toLowerCase().trim();

    let filtered = allSalesProducts;

    if (salesCatFilter !== 'Todos') {
        filtered = filtered.filter(p => (p.categoria || 'General') === salesCatFilter);
    }
    ...
    if (!filtered.length) {
        container.innerHTML = `
            <div style="grid-column:1/-1;text-align:center;padding:40px 20px;color:var(--text-muted);">
                <i class="fa-solid fa-box-open" style="font-size:2rem;opacity:0.3;display:block;margin-bottom:12px;"></i>
    ...
    container.innerHTML = filtered.map(product => {
        ...
        const imgSrc = product.imagen ? `/static/images/${product.imagen}` : null;
        ...
        return `
            <div class="product-card"
                ...
                <div class="product-card-body">
                    <div class="product-card-cat">${product.categoria || 'General'}</div>
                    <strong>${product.nombre}</strong>
                    <div class="product-card-meta">
                        <span class="product-card-price">${formatCurrency(product.precio)}</span>
                        <span class="product-card-stock ${isLowStock ? 'low' : ''}">
                ...
                <button type="button"
                        class="add-cart"
                        ${outOfStock ? 'disabled' : ''}
                        onclick="addToCartById(${product.id_producto})">
...
async function loadClientCart() {
    ...
    const statusEl = document.getElementById('cartClientLabel');
    ...
    if (!clientId) {
        ...
        renderCart();
        if (statusEl) {
            statusEl.className = 'sales-client-status';
            statusEl.innerHTML = '<i class="fa-solid fa-circle-info"></i> Ningún cliente seleccionado';
        }
        return;
    }
    ...
    if (statusEl) {
        statusEl.className = 'sales-client-status has-client';
        statusEl.innerHTML = `<i class="fa-solid fa-circle-check"></i> ${option.text}`;
    }
...
function renderCart() {
    ...
    // Compute totals with per-product IVA
    let subtotal = 0;
    let totalIva = 0;

    cart.forEach(item => {
        const base = item.precio * item.cantidad;
        const ivaPct = item.iva || 16;
        subtotal += base;
        totalIva += base * ivaPct / 100;
    });
    ...
    cartContent.innerHTML = cart.map(item => {
        ...
        <div class="cart-item" id="cart-item-${item.id}">
            <div class="cart-item-info">
                ...
                <div class="cart-item-price">${formatCurrency(item.precio)} c/u &nbsp;·&nbsp; IVA ${item.iva || 16}%</div>
            </div>
            <div class="cart-item-controls">
                <button type="button" class="btn-qty" onclick="updateQuantity(${item.id}, -1)">−</button>
                <span class="cart-item-qty">${item.cantidad}</span>
                <button type="button" class="btn-qty" onclick="updateQuantity(${item.id}, 1)">+</button>
                <button type="button" class="btn-remove-cart" onclick="removeFromCart(${item.id})" title="Eliminar">
    ...
    if (ivaEl) ivaEl.textContent = formatCurrency(totalIva);
...
document.addEventListener('click', async (e) => {
    if (e.target.closest('#generateInvoiceBtn')) {
        ...
        try {
            ...
            showToast(`✅ Factura INV-${String(response.invoice_id).padStart(3, '0')} creada exitosamente.`, 'success');
        } catch (error) {
            showToast(error.message, 'error');
        }
    }
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', async () => {
            ...
            if (!confirm(`¿Estás seguro de eliminar ${selectedOrderIds.length} pedido(s)? Esta acción es irreversible.`)) return;
            try {
