...
def api_create_materia_prima():
    ...
    try:
        conn.execute('''
            INSERT INTO materia_prima (fecha, unidades, tasa, precio_unitario, personalizado, iva, subtotal, categoria, talla, grupo_edad)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (fecha, unidades, tasa, precio_unitario, personalizado, iva, subtotal, categoria, talla, grupo_edad))
        conn.commit()
