...
def ensure_shared_db():
    ...
    # Custom migration for materia_prima
    try:
        conn.execute('''
        ...
        print("Error creating materia_prima table:", e)

    # Migration: add stock_disponible if not present
    try:
        conn.execute('ALTER TABLE materia_prima ADD COLUMN stock_disponible INTEGER DEFAULT NULL')
        conn.commit()
        # Seed stock_disponible from unidades for existing rows
        conn.execute('UPDATE materia_prima SET stock_disponible = unidades WHERE stock_disponible IS NULL')
        conn.commit()
    except Exception:
        pass
...
def map_product_row(row):
    return {
        ...
        'iva': float(row['iva']) if 'iva' in row and row['iva'] is not None else 16.0
    }
...
def fetch_products(categoria=None, search_query=None, sort_option='newest', limit=None):
    conn = get_shared_db()
    sql = (
        'SELECT p.id_producto, p.nombre, p.descripcion, p.precio_venta, p.iva, '
...
def fetch_product_by_id(product_id):
    conn = get_shared_db()
    row = conn.execute(
        'SELECT p.id_producto, p.nombre, p.descripcion, p.precio_venta, p.iva, c.nombre AS categoria, '
        'COALESCE((SELECT ruta_imagen FROM imagenes_productos ip WHERE ip.id_producto = p.id_producto ORDER BY ip.id_imagen LIMIT 1), ?) AS ruta_imagen, '
...
def api_product(product_id):
    ...
    product = conn.execute(
        'SELECT p.id_producto, p.nombre, p.descripcion, p.precio_venta AS precio, p.iva, '
...
def api_checkout():
    ...
    # Calculate subtotal and impuesto per item
    subtotal = 0.0
    impuesto = 0.0
    for item in cart_items:
        product_id = item.get('id')
        ...
        prod_iva = 16.0
        if product_id:
            prod = conn.execute('SELECT iva FROM productos WHERE id_producto = ? LIMIT 1', (product_id,)).fetchone()
            if prod and prod['iva'] is not None:
                prod_iva = float(prod['iva'])
    ...
    effective_iva = round((impuesto / subtotal) * 100, 2) if subtotal > 0 else 0.0
...
@admin_required
def api_get_materia_prima():
    conn = get_shared_db()
    try:
        rows = conn.execute('SELECT * FROM materia_prima WHERE activo = 1 ORDER BY fecha DESC').fetchall()
        items = [dict(row) for row in rows]
        return jsonify({'materia_prima': items})
    except Exception as e:
        return jsonify({'message': str(e)}), 500
    finally:
        conn.close()

@app.route('/api/materia-prima', methods=['POST'])
@admin_required
def api_create_materia_prima():
    data = request.get_json()
    if not data:
    ...
    personalizado = data.get('personalizado', '')
    iva = data.get('iva', 16.0)
    categoria = data.get('categoria')
    talla = data.get('talla', '')
    grupo_edad = data.get('grupo_edad', '')

    if not (fecha and unidades is not None and tasa is not None and precio_unitario is not None and categoria):
    ...
    try:
        ...
        iva = float(iva)
        base = unidades * precio_unitario
        subtotal = base + (base * iva / 100.0)
    except ValueError:
        return jsonify({'message': 'Tipos de datos inválidos para el cálculo.'}), 400

    conn = get_shared_db()
    try:
        conn.execute('''
            INSERT INTO materia_prima (fecha, unidades, tasa, precio_unitario, personalizado, iva, subtotal, categoria, talla, grupo_edad, stock_disponible)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (fecha, unidades, tasa, precio_unitario, personalizado, iva, subtotal, categoria, talla, grupo_edad, unidades))
        conn.commit()
        return jsonify({'message': 'Materia prima registrada correctamente.'}), 201
    except Exception as e:
        return jsonify({'message': str(e)}), 500
    finally:
        conn.close()

@app.route('/api/materia-prima/<int:id_materia>', methods=['PUT'])
@admin_required
def api_update_materia_prima(id_materia):
    data = request.get_json()
    if not data:
        return jsonify({'message': 'Datos no proporcionados.'}), 400
    ...
    conn = get_shared_db()
    try:
        row = conn.execute('SELECT id_materia FROM materia_prima WHERE id_materia = ? AND activo = 1', (id_materia,)).fetchone()
        if not row:
            return jsonify({'message': 'Materia prima no encontrada.'}), 404

        conn.execute('''
            UPDATE materia_prima
            SET fecha = ?, unidades = ?, tasa = ?, precio_unitario = ?, personalizado = ?, iva = ?, subtotal = ?, categoria = ?, talla = ?, grupo_edad = ?
            WHERE id_materia = ?
        ''', (fecha, unidades, tasa, precio_unitario, personalizado, iva, subtotal, categoria, talla, grupo_edad, id_materia))
        conn.commit()
        return jsonify({'message': 'Materia prima actualizada correctamente.'})
    except Exception as e:
        return jsonify({'message': str(e)}), 500
    finally:
        conn.close()

@app.route('/api/materia-prima/<int:id_materia>', methods=['DELETE'])
@admin_required
def api_delete_materia_prima(id_materia):
    conn = get_shared_db()
    try:
        row = conn.execute('SELECT id_materia FROM materia_prima WHERE id_materia = ? AND activo = 1', (id_materia,)).fetchone()
        if not row:
        ...
        # Soft delete
        conn.execute('UPDATE materia_prima SET activo = 0 WHERE id_materia = ?', (id_materia,))
        conn.commit()
        return jsonify({'message': 'Materia prima eliminada correctamente.'})
    except Exception as e:
        return jsonify({'message': str(e)}), 500
    finally:
        conn.close()
...
@admin_required
def api_consumir_materia_prima(id_materia):
    """Descontar unidades del stock disponible de materia prima."""
    data = request.get_json() or {}
    cantidad = data.get('cantidad', 0)
    motivo = data.get('motivo', 'personalizacion')
    ...
    try:
        row = conn.execute(
            'SELECT id_materia, unidades, stock_disponible, categoria FROM materia_prima WHERE id_materia = ? AND activo = 1',
            (id_materia,)
        ).fetchone()
        if not row:
            return jsonify({'message': 'Materia prima no encontrada.'}), 404

        stock_actual = row['stock_disponible'] if row['stock_disponible'] is not None else row['unidades']
        if cantidad > stock_actual:
            return jsonify({'message': f'Stock insuficiente. Disponible: {stock_actual} unidades.'}), 400

        nuevo_stock = stock_actual - cantidad
        conn.execute(
            'UPDATE materia_prima SET stock_disponible = ? WHERE id_materia = ?',
            (nuevo_stock, id_materia)
        )
        conn.commit()
        return jsonify({
            'message': f'Se descontaron {cantidad} unidad(es) de "{row["categoria"]}". Stock restante: {nuevo_stock}.',
...
def api_sales_data():
    ...
    products = conn.execute(
        'SELECT p.id_producto, p.nombre, p.precio_venta AS precio, '
        'IFNULL(i.stock_actual, 0) AS stock, '
        'p.iva, c.nombre AS categoria, '
        '(SELECT ip.ruta_imagen FROM imagenes_productos ip WHERE ip.id_producto = p.id_producto LIMIT 1) AS imagen '
        'FROM productos p '
        'LEFT JOIN inventario i ON i.id_producto = p.id_producto '
...
def api_create_product():
    ...
    iva = request.form.get('iva', 16.0, type=float)
    ...
    product_cursor = conn.execute(
        'INSERT INTO productos (nombre, descripcion, costo, precio_venta, id_categoria, activo, iva) VALUES (?, ?, ?, ?, ?, 1, ?)',
        (nombre, descripcion, precio, precio, category_id, iva)
    )
...
def api_update_product(product_id):
    ...
    conn.execute('UPDATE productos SET nombre = ?, descripcion = ?, precio_venta = ?, id_categoria = ?, iva = ? WHERE id_producto = ?',
                 (nombre, descripcion, precio, category_id, iva, product_id))
...
def api_admin_orders():
    ...
    orders = conn.execute(
        ...
        "env.direccion_envio, env.metodo_pago, env.referencia_pago, env.empresa_envio, env.numero_guia, env.comprobante_pago "
        "FROM pedidos p "
        "LEFT JOIN clientes c ON p.id_cliente = c.id_cliente "
        "LEFT JOIN estados_pedido e ON p.id_estado = e.id_estado "
        "LEFT JOIN envios env ON env.id_pedido = p.id_pedido "
...
def api_delete_orders():
    ...
    if not order_ids:
        return jsonify({'message': 'No se seleccionaron pedidos para eliminar.'}), 400

    conn = get_shared_db()
    try:
        placeholders = ','.join('?' for _ in order_ids)
        conn.execute(f'DELETE FROM detalle_pedidos WHERE id_pedido IN ({placeholders})', order_ids)
        conn.execute(f'DELETE FROM envios WHERE id_pedido IN ({placeholders})', order_ids)
        conn.execute(f'DELETE FROM pedidos WHERE id_pedido IN ({placeholders})', order_ids)
        conn.commit()
        conn.close()
        return jsonify({'message': 'Pedidos eliminados correctamente.'})
    except Exception as e:
        conn.close()
        return jsonify({'message': f'Error al eliminar pedidos: {str(e)}'}), 500
...
try:
    ...
    try:
        conn = get_shared_db()
        conn.execute('ALTER TABLE envios ADD COLUMN comprobante_pago VARCHAR(255)')
        conn.commit()
    except Exception:
        pass
    try:
        conn = get_shared_db()
        conn.execute('ALTER TABLE productos ADD COLUMN iva DECIMAL(5,2) DEFAULT 16.0')
        conn.commit()
    except Exception:
        pass
try:
...
def carrito():
    ...
    # Enrich session cart items with image_url and iva
    conn = get_shared_db()
    for item in cart:
        if item.get('id'):
            row = conn.execute(
                'SELECT ip.ruta_imagen, p.iva FROM productos p '
                'LEFT JOIN imagenes_productos ip ON ip.id_producto = p.id_producto '
                'WHERE p.id_producto = ? LIMIT 1',
                (item['id'],)
            ).fetchone()
            if row:
                item['image_url'] = row['ruta_imagen'] or 'placeholder.png'
                item['iva'] = float(row['iva']) if row['iva'] is not None else 16.0
            else:
                item['image_url'] = 'placeholder.png'
                item['iva'] = 16.0
        else:
    ...
    total_iva = sum(item['price'] * item.get('quantity', 1) * item.get('iva', 16.0) / 100 for item in cart)
    conn.close()
    return render_template('carrito.html', cart=cart, total=total, total_iva=total_iva, cart_count=len(cart))
...
def checkout():
    ...
    if request.method == 'POST' and request.form.get('name'):
        ...
        if screenshot and screenshot.filename:
            if validate_image_file(screenshot):
                ext = screenshot.filename.rsplit('.', 1)[-1].lower()
                screenshot_filename = f"payment_{uuid.uuid4().hex}.{ext}"
                uploads_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static', 'images', 'payments')
                os.makedirs(uploads_dir, exist_ok=True)
                screenshot.save(os.path.join(uploads_dir, screenshot_filename))
        ...
        conn.execute(
            'INSERT INTO envios (id_pedido, direccion_envio, empresa_envio, numero_guia, estado_envio, fecha_envio, metodo_pago, referencia_pago, tipo_envio, comprobante_pago) VALUES (?, ?, ?, ?, ?, datetime("now"), ?, ?, ?, ?)',
            (pedido_id, address, envio_desc, '', 'Pendiente', payment_method, reference, shipping_method, screenshot_filename)
        )
    ...
    for item in cart:
        ...
        if product_id:
            ...
            if prod and prod['iva'] is not None:
                prod_iva = float(prod['iva'])
        iva_amount += price * qty * prod_iva / 100
    conn.close()

    return render_template('checkout.html', total=total, iva_amount=iva_amount, cart_count=cart_count, item_count=len(cart))
...
def factura(order_id):
    ...
    items = conn.execute(
        'SELECT dp.cantidad, dp.precio_unitario, pr.nombre AS name, pr.iva '
        'FROM detalle_pedidos dp '
        'LEFT JOIN productos pr ON dp.id_producto = pr.id_producto '
        'WHERE dp.id_pedido = ?',
        (order_id,)
    ...
    for item in items:
        ...
        price = float(item['precio_unitario'])
        prod_iva = float(item['iva']) if item['iva'] is not None else 16.0
        item_subtotal = price * qty
        items_subtotal += item_subtotal
        iva_amount += item_subtotal * prod_iva / 100

    iva_amount = round(iva_amount, 2)
    total_final = round(items_subtotal + iva_amount, 2)
    effective_iva = round((iva_amount / items_subtotal) * 100, 2) if items_subtotal > 0 else 0.0
